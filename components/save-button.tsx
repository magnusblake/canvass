...
export default function SaveButton({ projectId, className }: SaveButtonProps) {
  ...
  if (!initialCheckDone) {
    return (
      <Button variant="outline" size="icon" className={cn("w-10 h-10 rounded-full", className)} disabled>
  ...
  return (
    <Button
      variant="outline"
      size="icon"
      className={cn("w-10 h-10 rounded-full", isSaved && "bg-primary/10 text-primary border-primary/50", className)}
      onClick={handleSave}
      disabled={isLoading}
