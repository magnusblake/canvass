...
export default function ProfileImageUpload({ type, currentUrl, onUpload, className }: ProfileImageUploadProps) {
  ...
  if (type === "avatar") {
    return (
      <div className={cn("relative w-24 h-24 rounded-full overflow-hidden group", className)}>
        <Image src={previewUrl || `/api/avatar?name=User`} alt="Аватар" fill className="object-cover" />
  ...
  return (
    <div className={cn("relative w-full h-40 rounded-lg overflow-hidden bg-muted", className)}>
      {previewUrl && <Image src={previewUrl} alt="Баннер" fill className="object-cover" />}
      <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
        <label>
          ...
          <Button variant="outline" className="bg-white/80 hover:bg-white">
            {isUploading ? <RefreshCw className="h-4 w-4 mr-2 animate-spin" /> : <Camera className="h-4 w-4 mr-2" />}
